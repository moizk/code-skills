---
name: copy-component
description: "Use when an instruction points at an existing component, screen, feature, or behavior as the model to match — 'copy the X from Y', 'mirror X', 'port X', 'make it like the existing screen', 'same as the other page', 'reuse the styles and js from X'. Faithfully ports it so the copy has the SAME behaviors and mechanism as its source, not merely the same look: reads the source first, inventories every behavior (markup, styles, JS, keyboard/focus/aria, positioning, states), reuses the source's mechanism instead of hand-rolling a substitute, surfaces omissions as explicit deviations, and diffs against the source before calling it done. Not for net-new components with no in-repo source (use ui-ux-plan) or one-line tweaks."
---
